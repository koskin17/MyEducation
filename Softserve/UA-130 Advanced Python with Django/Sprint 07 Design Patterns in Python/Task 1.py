...
# Ты абсолютно прав — технически можно **не использовать `ABC`**, а просто сделать класс-заглушку с `pass`. Но давай разберёмся, **в чём разница между обычной заглушкой и настоящим абстрактным классом `ABC`**, и **почему `ABC` — лучше**.

## 🔍 Что ты предлагаешь:
# class Product:
#     def cook(self):
#         pass
# Это **не настоящий абстрактный класс**, а просто класс, в котором метод `cook()` — заглушка. Да, такой код будет работать.
# Но **есть три причины**, почему это не лучшее решение ⤵️

## ✅ Зачем нужен `ABC` и почему он лучше?
# from abc import ABC, abstractmethod
...
Теперь по шагам:
### 1. 🛡️ Защита от ошибок
# Если ты **забыл реализовать метод `cook()` в дочернем классе**, то:
#### ❌ Без `ABC`:
# class Tiramisu(Product):  # ничего не реализуем
#     pass

# dish = Tiramisu()
# dish.cook()  # 🤷 работает, но ничего не делает. Баг скрыт!
...
# dish = Tiramisu()  # 💥 Ошибка!

# > `TypeError: Can't instantiate abstract class Tiramisu with abstract method cook`
# 👉 **Python сам остановит тебя**, если ты что-то забыл — это как страховка от дурака 😅

### 2. 🧠 Явное намерение
# Когда ты пишешь:
# class Product(ABC):
# Это говорит всем:
# > "Этот класс — **абстрактный**, его не надо создавать напрямую. Он только для наследников."
# Это **самодокументирующийся код** — сразу видно архитектуру.

### 3. 🧩 Стандарты и поддержка
# * `ABC` — это **официальный способ** создавать абстрактные классы в Python (и аналог интерфейсов в Java, C#).
# * Абстрактные классы работают **в связке с type checking** (например, с `mypy` или `pylint`) и помогают IDE подсказывать ошибки.

## 💬 Простыми словами
# Представь, что ты строишь дом:
# * `ABC` — это как архитекторский чертёж: без него ты **не можешь начать стройку**, пока всё не запланировано.
# * `pass` — это как "давай потом доделаем" — и ты можешь забыть, где чего не хватает 🤷

## ✨ Вывод

# | Вопрос                        | Без `ABC`                     | С `ABC`                            |
# | ----------------------------- | ----------------------------- | ---------------------------------- |
# | Заставляет реализовать метод  | ❌ Нет                         | ✅ Да                               |
# | Помогает находить ошибки      | ❌ Нет                         | ✅ Да (ошибка при создании объекта) |
# | Явно говорит о структуре      | ❌ Нет (загадка для читающего) | ✅ Да (архитектурно понятно)        |
# | Поддерживается IDE/type-check | ❌ Не всегда                   | ✅ Да (встроенная поддержка)        |
